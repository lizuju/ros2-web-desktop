...
echo "Configure Jetson noVNC."
echo "Press Enter to accept the suggested value."
echo

ros_domain_id="$(read_value "ROS_DOMAIN_ID" "0")"
jetson_ros_setup="$(read_value "Absolute robot workspace setup file, or leave empty" "")"
...
awk -v ros_domain_id="$ros_domain_id" \
    ...
    'BEGIN {
     ...
     END {
       ...
       if (!seen_jetson_ros_setup) print "JETSON_ROS_SETUP=" jetson_ros_setup
...
echo "  JETSON_ROS_SETUP=${jetson_ros_setup}"
...
echo "  ssh -N -L 18080:127.0.0.1:${novnc_port} <jetson-user>@<jetson-ip>"
echo
echo "Then open:"
